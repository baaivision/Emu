...
class VQA:
    ...
    def getQuesIds(self, imgIds=[], quesTypes=[], ansTypes=[]):
        ...
        # ids = [int(ann["question_id"].item()) for ann in anns]
        return ids
    ...
    def loadRes(self, resFile, quesFile):
        ...
        print(f"quesFile: {quesFile}, resFile: {resFile}")
        assert type(anns) == list, "results is not an array of objects"
        annsQuesIds = [ann["question_id"] for ann in anns]
        print(len(annsQuesIds), annsQuesIds[:10])
        print(len(self.getQuesIds()), self.getQuesIds()[:10])
